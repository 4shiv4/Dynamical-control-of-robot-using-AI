...
id_robot_2=bullet.loadURDF("D:\\UGP-1\\svan_RR_leg_rig_urdf\\urdf\\svan_RR_leg_rig_urdf.urdf",[0., 1., 0.], robot_orientation,useFixedBase=True)
id_robot=bullet.loadURDF("D:\\UGP-1\\svan_RR_leg_rig_urdf\\urdf\\svan_RR_leg_rig_urdf.urdf",[1.,0.,0.5], robot_orientation,useFixedBase=True)
...
bullet.changeDynamics(id_robot, 2, linearDamping=0, angularDamping=0)
...
q_acc_desired = [[0.] * steps, [0.] * steps, [0.] * steps]
k=10
for s in range(steps):
  t[s] = start + s * delta_t
  q_pos_desired[0][s] = 1. / (2. * math.pi) * math.sin(2. * math.pi * t[s]/k )#- t[s]       
  q_pos_desired[1][s] = -1. / (2. * math.pi) * (math.cos(2. * math.pi * t[s]/k) - 1.0)

  q_vel_desired[0][s] = (math.cos(2. * math.pi * t[s]/k))/k #- 1.
  q_vel_desired[1][s] = math.sin(2. * math.pi * t[s]/k)/k
  
  q_acc_desired[0][s]= 0
  q_acc_desired[1][s] = -2. * math.pi * math.sin(2. * math.pi * t[s]/k)/(k*k)
  q_acc_desired[2][s] = 2. * math.pi * math.cos(2. * math.pi * t[s]/k)/(k*k)
...
for i in range(len(t)):
  if i==0:
    bullet.resetJointState(id_robot, 1,bullet.getJointStates(id_robot, id_revolute_joints)[1][0],1/k)
...
#   mass_matrix = calculateMassMatrix(id_robot, q_pos)
  q_vel[0][i] = joint_states[0][1]
  q_vel[1][i] = joint_states[1][1]
  q_vel[2][i] = joint_states[2][1]
  
  # Computing the torque from inverse dynamics:
  obj_pos = [q_pos[0][i], q_pos[1][i], q_pos[2][i]]
  obj_vel = [q_vel[0][i], q_vel[1][i], q_vel[2][i]]
  obj_acc = [q_acc_desired[0][i], q_acc_desired[1][i], q_acc_desired[2][i]]
  obj_acc_test = [q_acc_desired[0][i], q_acc_desired[2][i], q_acc_desired[1][i]]
...
#                                    [1,2],
#                                    bullet.TORQUE_CONTROL,
#                                    forces=[torque[1], torque[2]])

  # Step Simulation
  bullet.stepSimulation()
id_revolute_joints_2=[0,1,2]
...
for s in range(steps):
  t[s] = start + s * delta_t
  q_pos_desired_test[0][s] = 0.56* math.sin(2. * math.pi * t[s]/k ) + 0.7
  q_pos_desired_test[1][s] = -0.8 * (math.sin(2. * math.pi * t[s]/k)) - 1.2

  q_vel_desired_test[0][s] = 0.56*2.*math.pi*(math.cos(2. * math.pi * t[s]/k))/k
  q_vel_desired_test[1][s] = -0.8*2.*math.pi*math.cos(2. * math.pi * t[s]/k)/k
  
  q_acc_desired_test[0][s]= 0
  q_acc_desired_test[1][s] = -0.56*(2. * math.pi)*(2. * math.pi) * math.sin(2. * math.pi * t[s]/k)/(k*k)
  q_acc_desired_test[2][s] = 0.8*(2. * math.pi) *(2.*math.pi) * math.sin(2. * math.pi * t[s]/k)/(k*k)
# Do Torque Control:
...
for i in range(len(t)):
  ...
  # Read Sensor States:
  joint_states = bullet.getJointStates(id_robot_2, id_revolute_joints_2)
...
#                                    forces=[0, torque_test[1], torque_test[2]])
  ...
  bullet.stepSimulation()
#   time.sleep(1/240)
# bullet.stopStateLogging(log_id)

pybullet_H = [[[0, 0, 0] for _ in range(3)] for _ in range(steps)]
# plt.plot(q_pos_test[0])
# plt.plot(q_pos_test[1])
# plt.plot(q_pos_test[2])
# plt.show()
for i in range(steps):
    # pybullet_H[i] = bullet.calculateMassMatrix(id_robot, [q_pos_test[0][i],q_pos_desired_test[0][i],q_pos_desired_test[1][i]])
    pybullet_H[i] = bullet.calculateMassMatrix(id_robot_2, [q_pos_test[0][i],q_pos_test[1][i],q_pos_test[2][i]])
# print(pybullet_H[0][:][0][0])
# print([pybullet_H[step][0][0] for step in range(steps)])

# plt.plot([pybullet_H[step][0][0] for step in range(steps)])
# plt.plot([pybullet_H[step][0][1] for step in range(steps)])
# plt.plot([pybullet_H[step][0][2] for step in range(steps)])
# plt.plot([pybullet_H[step][1][0] for step in range(steps)])
# plt.plot([pybullet_H[step][1][1] for step in range(steps)])
# plt.plot([pybullet_H[step][1][2] for step in range(steps)])
# plt.plot([pybullet_H[step][2][0] for step in range(steps)])
# plt.plot([pybullet_H[step][2][1] for step in range(steps)])
# plt.plot([pybullet_H[step][2][2] for step in range(steps)])
# plt.legend(["1,2","1,3","2,1","2,2","2,3","3,1","3,2","3,3"])
# plt.show()
...
E22=np.zeros(steps)
for i in range(steps):
    E01[i]=m1*l1c*m.sin(q_pos_desired_test[0][i])+m2*(l1*m.cos(q_pos_desired_test[0][i])-l2c*m.cos(q_pos_desired_test[1][i]))
    E02[i]=m2*l2c*m.cos(q_pos_desired_test[1][i])
    # E11[i]=i1+i2+m1*l1c*l1c+2*m2*(((l1*m.cos(q_pos_desired_test[0][i]))**2)+((l2c*m.cos(q_pos_desired_test[1][i]-q_pos_desired_test[0][i]))**2))
    E11[i]=i1+i2+m1*l1c*l1c+m2*l1*l1+m2*l2c*l2c+2*m2*l1*l2c*m.cos(q_pos_desired_test[1][i])
    # E12[i]=i2+2*m2*((l2c*m.cos(q_pos_desired_test[1][i]-q_pos_desired_test[0][i]))**2)
    E12[i]=i2+m2*l1*l2c*m.cos(q_pos_desired_test[1][i])+m2*l2c*l2c
    E22[i]=i2+m2*l2c*l2c
...
#         if i==0 and j==1:
#             ax.plot(E01,label='Analytical H')
#         if i==0 and j==2:
#             ax.plot(E02,label='Analytical H')
#         if i==1 and j==1:
#             ax.plot(E11,label='Analytical H')
#         if i==1 and j==2:
#             ax.plot(E12,label='Analytical H')
#         if i==2 and j==2:
#             ax.plot(E22,label='Analytical H')
...
#   ax_tor.plot(t, q_tor_test[0], '-g', lw=2, label='Torque q0')
  ax_tor.plot(t, q_tor_test[1], '-r', lw=2, label='Torque q1')
  ax_tor.plot(t, q_tor_test[2], '-b', lw=2, label='Torque q2')
...
if __name__ == "__main__":
    ...
    # print(np.shape(train_qp))
    # print(np.shape(train_qv))
    # print(np.shape(train_qa))
    # print(np.shape(train_tau))

    train_qp = q_pos_desired
    train_qv = q_vel_desired
    train_qa = [q_acc_desired[1], q_acc_desired[2]]
    train_tau = [q_tor[1], q_tor[2]]

    train_qp = np.array(train_qp)
    train_qv = np.array(train_qv)
    train_qa = np.array(train_qa)
    train_tau = np.array(train_tau)

    train_qp = np.transpose(train_qp)
    train_qv = np.transpose(train_qv)
    train_qa = np.transpose(train_qa)
    train_tau = np.transpose(train_tau)
    ...
    test_qp = [q_pos_desired_test[0], q_pos_desired_test[1]]
    test_qv = [q_vel_desired_test[0], q_vel_desired_test[1]]
    test_qa = [q_acc_desired_test[1], q_acc_desired_test[2]]
    # test_tau=train_tau
    test_tau = [q_tor_test[1], q_tor_test[2]]

    test_qp = np.array(test_qp)
    test_qv = np.array(test_qv)
    test_qa = np.array(test_qa)
    test_tau = np.array(test_tau)

    test_qp = np.transpose(test_qp)
    test_qv = np.transpose(test_qv)
    test_qa = np.transpose(test_qa)
    test_tau = np.transpose(test_tau)

    # print('hi',np.shape(test_qp))
    # print(np.shape(test_qv))
    # print(np.shape(test_qa))
    # print(np.shape(test_tau))
...
# # Adjust layout for better spacing
#     plt.tight_layout()
#     plt.savefig('D:\\UGP-1\\Delan_vs_pybullet_H.png')
#     plt.show()
...
# id_robot3=bullet.loadURDF("D:\\UGP-1\\svan_RR_leg_rig_urdf\\urdf\\svan_RR_leg_rig_urdf.urdf",robot_base, robot_orientation,useFixedBase=True)
...
q_vel = [[0.] * steps, [0.] * steps, [0.] * steps]
q_tor = [[0.] * steps, [0.] * steps, [0.] * steps]
# log_id = bullet.startStateLogging(bullet.STATE_LOGGING_VIDEO_MP4, "D:\\UGP-1\\simulation_delan.mp4")
...
for i in range(len(t)):
  ...
  if (verbose):
    ...
    print("obj_acc")
    print(obj_acc)
#   print(np.shape(test_tau[i]),np.shape(test_tau))

  torque = [q_tor_test[0][i], delan_tau[i][0], delan_tau[i][1]]
...
#   ax_pos.set_ylim(-1., 1.)
  ...
  ax_vel.plot(t, q_vel_desired_test[0], '--r', lw=2, label='Desired q1')
  ax_vel.plot(t, q_vel_desired_test[1], '--b', lw=2, label='Desired q2')
  ax_vel.plot(t, q_vel[1], '-r', lw=1, label='Measured q1')
  ax_vel.plot(t, q_vel[2], '-b', lw=1, label='Measured q2')
#   ax_vel.set_ylim(-2., 2.)
  ax_vel.legend()
...
#   ax_tor.plot(t, q_tor[0], '-g', lw=2, label='Torque q0')
  ax_tor.plot(t, q_tor[1], '-r', lw=2, label='Delan Torque q1')
  ax_tor.plot(t, q_tor[2], '-b', lw=2, label='Delan Torque q2')
  ax_tor.plot(t, q_tor_test[1], '--r', lw=2, label='Actual Torque q1')
  ax_tor.plot(t, q_tor_test[2], '--b', lw=2, label='Actual Torque q2')
#   ax_tor.set_ylim(-20., 20.)
  ax_tor.legend()
...
#   ax_pos = figure.add_subplot(141)
#   ax_pos.set_title("Joint Position")
#   ax_pos.plot(t, q_pos_desired[1], '--r', lw=2, label='Desired q1')
#   ax_pos.plot(t, q_pos_desired[0], '--b', lw=2, label='Desired q2')
#   ax_pos.plot(t, q_pos_test[1], '-r', lw=1, label='Measured q1')
#   ax_pos.plot(t, q_pos_test[2], '-b', lw=1, label='Measured q2')
#   ax_pos.set_ylim(-1., 1.)
#   ax_pos.legend()
...
#   ax_vel = figure.add_subplot(142)
#   ax_vel.set_title("Joint Velocity")
#   ax_vel.plot(t, q_vel_desired[1], '--r', lw=2, label='Desired q1')
#   ax_vel.plot(t, q_vel_desired[0], '--b', lw=2, label='Desired q2')
#   ax_vel.plot(t, q_vel_test[1], '-r', lw=1, label='Measured q1')
#   ax_vel.plot(t, q_vel_test[2], '-b', lw=1, label='Measured q2')
#   ax_vel.set_ylim(-2., 2.)
#   ax_vel.legend()
...
#   ax_acc = figure.add_subplot(143)
#   ax_acc.set_title("Joint Acceleration")
#   ax_acc.plot(t, q_acc_desired[1], '--r', lw=4, label='Desired q1')
#   ax_acc.plot(t, q_acc_desired[2], '--b', lw=4, label='Desired q2')
#   ax_acc.set_ylim(-10., 10.)
#   ax_acc.legend()
